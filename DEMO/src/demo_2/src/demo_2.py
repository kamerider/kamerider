...
class Demo:
	...
	while not rospy.is_shutdown():
	  ...
	  if start == 1:
		...
		if message == "origin":
		  ...
		  if waiting == 1:
		    ...
		    #rospy.sleep(2)
		    if again == 3:
		    	self.soundhandle.say("Here is your bottle", self.voice)
		    	rospy.sleep(2)
		    	global start
		    	start = 10
		    else :
	  ...
	  if start == 3:
		...
		if start != 10:
              	    self.message.linear.x = 0.10; self.message.linear.y = 0; self.message.linear.z = 0
              	    self.message.angular.x = 0.0; self.message.angular.y = 0; self.message.angular.z = 0
              	    self.pub.publish(self.message)
    ...
    def callback(self, data):
	    ...
	    elif( blob_position_x >= 310 and blob_position_x <= 325):
		if count == 0:
			global again
			again = again + 1
		...
		if again == 1:
			...
			while start != 2:
				rospy.sleep(0.5)
		    	self.soundhandle.say("Found bottle", self.voice)
		   	rospy.sleep(2)
		    	self.soundhandle.say("Try to get bottle", self.voice)
		   	rospy.sleep(2)
			global again
			again = again + 1
			global count
